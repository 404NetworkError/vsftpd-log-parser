...
while (<FH>) {
	...
	if ( $action =~ /LOGIN:/ ) {
		$hits_by_login_for{$status}++;
		if ( $status =~ /OK/ ) {
			$hits_by_status_ok_for{$ip}++;
		}
		if ( $status =~ /FAIL/ ) {
			$hits_by_status_fail_for{$ip}++;
		}
	}
	...
	# print "$day\t$time\t$user\t$ip\t$file\t$status\t$action\t$b\n"; # debugging purposes
...
Display(\%hits_by_login_for,   'By Login Status',         $max_records);
